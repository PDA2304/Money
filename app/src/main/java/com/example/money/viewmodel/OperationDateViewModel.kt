...
class OperationDateViewModel(val context: View) : ViewModel() {
    ...
    fun onOperationDateAll(type: Int, date_from: Date,date_to: Date?, Invoice_ID  : Int, arrayTextView: ArrayList<TextView>): ArrayList<OperationDate> {
        val array = data.OperationDateAll(type, date_from,date_to,Invoice_ID)
        if (array.size == 0) {
